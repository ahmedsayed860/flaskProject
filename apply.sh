...
original_author="ahmedsayed860"
original_name="flaskproject"
original_urlname="flaskProject"
original_description="Awesome flaskproject created by ahmedsayed860" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flaskproject "${name}"
...
then
    ...
    echo "then run 'flaskproject' or 'python -m flaskproject'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flaskproject' or 'python -m flaskproject' or 'poetry run flaskproject'"
fi
