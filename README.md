...
- 🎯 Entry points to execute your program using `python -m <flaskproject>` or `$ flaskproject` with basic CLI argument parsing.
...
---
# flaskproject Flask Application

Awesome flaskproject created by ahmedsayed860
...
```bash
git clone https://github.com/ahmedsayed860/flaskProject flaskproject
cd flaskproject
make install
```
...
```bash
flaskproject create-db   # run once
flaskproject populate-db  # run once (optional)
flaskproject add-user -u admin -p 1234  # ads a user
flaskproject run
```
